...
class CartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        # This is no longer used as a termination condition
    ...
    def step(self, action):
        ...
        if not terminated:
            ...
            # reward = max(0, math.cos(theta))
            # reward = 0.1 * math.cos(theta)  # shaping term
            # if abs(theta) < np.pi / 2:
            #     reward += 1.0  # bonus for being in the upright zone

            if abs(theta) < 0.05:  # upright zone
                reward = 3.0
            else:
                reward = 0.5 * (1 - abs(theta) / (np.pi / 2))
...
gym.register(id="custom_cartpole", entry_point=CartPoleEnv, max_episode_steps=500)
